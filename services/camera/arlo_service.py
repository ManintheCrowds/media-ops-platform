...
class ArloService:
                    ...
                    cameras.append(
                        {
                            ...
                            "name": device.get(
                                "deviceName", f"Camera {device.get('deviceId')}"
                            ),
                            "status": (
                                ArloStatus.ONLINE.value
                                if device.get("state") == "provisioned"
                                else ArloStatus.OFFLINE.value
                            ),
                            "is_armed": device.get("properties", {}).get(
                                "armed", False
                            ),
                            "battery_level": device.get("properties", {}).get(
                                "batteryLevel", None
                            ),
                            "signal_strength": device.get("properties", {}).get(
                                "signalStrength", None
                            ),
                        }
                    )
    ...
    async def register_camera(self, db: Session, camera_data: Dict) -> Dict:
        ...
        existing = (
            db.query(ArloCamera).filter_by(device_id=camera_data["device_id"]).first()
        )
        if existing:
            ...
            existing.status = (
                ArloStatus[status_str]
                if hasattr(ArloStatus, status_str)
                else ArloStatus.UNKNOWN
            )
    ...
    async def get_camera(self, db: Session, camera_id: int) -> Dict:
        ...
        if not camera:
            raise ArloCameraError(
                f"Camera not found: {camera_id}", camera_id=str(camera_id)
            )
        return camera.to_dict()
    ...
    async def arm_camera(self, db: Session, camera_id: int) -> Dict:
        ...
        try:
            ...
            raise ArloCameraError(
                f"Failed to arm camera: {str(e)}", camera_id=str(camera_id)
            )
    ...
    async def disarm_camera(self, db: Session, camera_id: int) -> Dict:
        ...
        try:
            ...
            raise ArloCameraError(
                f"Failed to disarm camera: {str(e)}", camera_id=str(camera_id)
            )
    ...
    async def capture_snapshot(self, db: Session, camera_id: int) -> Dict:
        ...
        try:
            ...
            raise ArloCameraError(
                f"Failed to capture snapshot: {str(e)}", camera_id=str(camera_id)
            )
    ...
    async def get_library(
        ...
        try:
            ...
            for recording in library[:limit]:
                ...
                existing = (
                    db.query(ArloRecording)
                    .filter_by(recording_id=recording.get("name"))
                    .first()
                )
                ...
                if not existing:
                    ...
                    recording_obj = ArloRecording(
                        ...
                        created_date=datetime.fromtimestamp(
                            recording.get("utcCreatedDate", 0) / 1000
                        ),
            ...
            raise ArloRecordingError(
                f"Failed to get library: {str(e)}", camera_id=str(camera_id)
            )

    async def download_recording(
        self, db: Session, camera_id: int, recording_id: int
    ) -> Dict:
        ...
        recording = (
            db.query(ArloRecording)
            .filter_by(camera_id=camera.id, id=recording_id)
            .first()
        )

        if not recording:
            raise ArloRecordingError(
                f"Recording not found: {recording_id}", camera_id=str(camera_id)
            )
    ...
    async def delete_recording(
        ...
        try:
            ...
            raise ArloRecordingError(
                f"Failed to delete recording: {str(e)}", camera_id=str(camera_id)
            )
    ...
    async def get_camera_status(self, db: Session, camera_id: int) -> Dict:
        ...
        try:
            ...
            devices = arlo.GetDevices()
            device = next(
                (d for d in devices if d.get("deviceId") == camera.device_id), None
            )
            ...
            if device:
                ...
                camera.status = (
                    ArloStatus.ONLINE
                    if device.get("state") == "provisioned"
                    else ArloStatus.OFFLINE
                )
                camera.battery_level = device.get("properties", {}).get("batteryLevel")
                camera.signal_strength = device.get("properties", {}).get(
                    "signalStrength"
                )
                db.commit()
            ...
            raise ArloCameraError(
                f"Failed to get camera status: {str(e)}", camera_id=str(camera_id)
            )
    ...
    async def start_live_stream(self, db: Session, camera_id: int) -> Dict:
        ...
        try:
            ...
            raise ArloCameraError(
                f"Failed to start live stream: {str(e)}", camera_id=str(camera_id)
            )
                ...
                for url in urls_to_try:
                    try:
                        async with session.get(url, allow_redirects=False) as response:
                            ...
                            if "arlo" in server_header or response.status in [
                                200,
                                401,
                                403,
                            ]:
                                ...
                                try:
                                    ...
                                    logger.debug(
                                        f"Could not read response text for {ip}: {e}"
                                    )
                                    pass
                        ...
                        logger.warning(
                            f"Unexpected error probing {ip}: {e}", exc_info=True
                        )
                        continue
