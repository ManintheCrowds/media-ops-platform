...
class Arlo(object):
    ...
    def genTransId(self, trans_type=TRANSID_PREFIX):
        ...
        return (
            trans_type
            + "!"
            + float2hex(random.random() * math.pow(2, 32)).lower()
            + "!"
            + str(int((time.mktime(now.timetuple()) * 1e3 + now.microsecond / 1e3)))
        )
    ...
    def Login(self, username, password):
        ...
        body = self.request.post(
            ...
            params={
                "email": self.username,
                "password": str(
                    base64.b64encode(self.password.encode("utf-8")), "utf-8"
                ),
                "language": "en",
                "EnvSource": "prod",
            },
    ...
    def LoginMFA(self, username, password, google_credential_file):
        ...
        self.request.session.headers.update(
            {
                "Authorization": base64.b64encode(
                    auth_body["data"]["token"].encode("utf-8")
                )
            }
        )
        ...
        email_factor = next(
            (
                i
                for i in factors_body["data"]["items"]
                if i["factorType"] == "EMAIL" and i["factorRole"] == "PRIMARY"
            ),
            None,
        )
        if email_factor is None:
            email_factor = next(
                (
                    i
                    for i in factors_body["data"]["items"]
                    if i["factorType"] == "EMAIL" and i["factorRole"] == "SECONDARY"
                ),
                None,
            )
        ...
        for i in range(0, 10):
            ...
            message = (
                service.users()
                .messages()
                .get(userId="me", id=messages["messages"][0]["id"])
                .execute()
            )
    ...
    def Subscribe(self, basestation):
        ...
        def Register(self):
            if (
                self.event_stream
                and self.event_stream.connected
                and not self.event_stream.registered
            ):
                self.Notify(
                    basestation,
                    {
        ...
        if not self.event_stream or not self.event_stream.connected:
            ...
            while (
                not self.event_stream.connected
                and not self.event_stream.event_stream_stop_event.is_set()
            ):
                time.sleep(0.5)
    ...
    def Notify(self, basestation, body):
        ...
        self.request.post(
            f"https://{self.BASE_URL}/hmsweb/users/devices/notify/" + body["to"],
            body,
    ...
    def NotifyAndGetResponse(self, basestation, body, timeout=120):
        ...
        if (
            self.event_stream
            and self.event_stream.connected
            and self.event_stream.registered
        ):
    ...
    def Ping(self, basestation):
        basestation_id = basestation.get("deviceId")
        return self.NotifyAndGetResponse(
            basestation,
            {
                "action": "set",
                "resource": "subscriptions/" + self.user_id + "_web",
    ...
    def GetSmartFeatures(self):
        return self.request.get(
            f"https://{self.BASE_URL}/hmsweb/users/subscription/smart/features"
        )

    def GetSmartAlerts(self, camera):
        return self.request.get(
            f"https://{self.BASE_URL}/hmsweb/users/devices/"
            + camera.get("uniqueId")
            + "/smartalerts"
        )
    ...
    def RestartBasestation(self, basestation):
        return self.request.post(
            f"https://{self.BASE_URL}/hmsweb/users/devices/restart",
    ...
    def GetAutomationDefinitions(self):
        return self.request.get(
            f"https://{self.BASE_URL}/hmsweb/users/automation/definitions",
            {"uniqueIds": "all"},
        )
    ...
    def DeleteMode(self, device, mode):
        ...
        if device.get("deviceType") == "arlobridge":
            return self.request.delete(
                f"https://{self.BASE_URL}/hmsweb/users/locations/"
                + device.get("uniqueId")
                + "/modes/"
                + mode
            )
    ...
    def GetModesV2(self):
        ...
        """
        return self.request.get(
            f"https://{self.BASE_URL}/hmsweb/users/devices/automation/active"
        )
    ...
    def SetMotionAlertsOn(self, basestation, sensitivity=5):
        return self.NotifyAndGetResponse(
            ...
            {
                "action": "set",
                "resource": "cameras/" + basestation.get("deviceId"),
                "publishResponse": True,
                "properties": {
                    "motionDetection": {
                        "armed": True,
    ...
    def AlertNotificationMethods(
        self, basestation, action="disabled", email=False, push=False
    ):
    ...
    def PlayTrack(
        self, basestation, track_id="2391d620-e491-4412-99f6-e9a40d6046ed", position=0
    ):
        """
    ...
    def PauseTrack(self, basestation):
        return self.Notify(
            basestation, {"action": "pause", "resource": "audioPlayback/player"}
        )

    def UnPauseTrack(self, basestation):
        return self.Notify(
            basestation, {"action": "play", "resource": "audioPlayback/player"}
        )

    def SkipTrack(self, basestation):
        return self.Notify(
            basestation, {"action": "nextTrack", "resource": "audioPlayback/player"}
        )

    def SetSleepTimerOn(
        self, basestation, time=calendar.timegm(time.gmtime()) + 300, timediff=0
    ):
        return self.NotifyAndGetResponse(
            basestation,
            {
                ...
                "publishResponse": True,
                "properties": {"config": {"sleepTime": time, "sleepTimeRel": timediff}},
            },
        )
    ...
    def SetNightLightMode(self, basestation, mode="rainbow"):
        ...
        return self.NotifyAndGetResponse(
            basestation,
            {
                ...
                "resource": "cameras/" + basestation.get("deviceId"),
                "publishResponse": True,
                "properties": {"nightLight": {"mode": mode}},
            },
        )
    ...
    def SetNightLightTimerOn(
        ...
        return self.NotifyAndGetResponse(
            basestation,
            {
                ...
                "publishResponse": True,
                "properties": {
                    "nightLight": {"sleepTime": time, "sleepTimeRel": timediff}
                },
            },
        )
    ...
    def SetTempUnit(self, uniqueId, unit="C"):
        return self.request.post(
            f"https://{self.BASE_URL}/hmsweb/users/devices/" + uniqueId + "/tempUnit",
            {"tempUnit": unit},
        )
    ...
    def GetServiceLevelSettings(self):
        return self.request.get(
            f"https://{self.BASE_URL}/hmsweb/users/serviceLevel/settings"
        )
    ...
    def GetUpdateFeatures(self):
        return self.request.get(
            f"https://{self.BASE_URL}/hmsweb/users/devices/updateFeatures/feature"
        )

    def GetPaymentBilling(self):
        return self.request.get(
            f"https://{self.BASE_URL}/hmsweb/users/payment/billing/" + self.user_id
        )
    ...
    def GetPaymentOffersV2(self):
        """DEPRECATED: This API still works, but I don't see it being called in the web UI anymore."""
        return self.request.get(
            f"https://{self.BASE_URL}/hmsweb/users/payment/offers/v2"
        )

    def GetPaymentOffersV3(self):
        """DEPRECATED: This API still works, but I don't see it being called in the web UI anymore."""
        return self.request.get(
            f"https://{self.BASE_URL}/hmsweb/users/payment/offers/v3"
        )

    def GetPaymentOffersV4(self):
        return self.request.get(
            f"https://{self.BASE_URL}/hmsweb/users/payment/offers/v4"
        )

    def SetOCProfile(
        self, firstName, lastName, country="United States", language="en", spam_me=0
    ):
        return self.request.post(
            f"https://{self.BASE_URL}/hmsweb/users/ocprofile",
            {
                "firstName": "Jeffrey",
                "lastName": "Walter",
                "country": country,
    ...
    def GetEmergencyLocations(self):
        return self.request.get(
            f"https://{self.BASE_URL}/hmsweb/users/emergency/locations"
        )
    ...
    def Geofencing(self, location_id, active=True):
        ...
        return self.request.put(
            f"https://{self.BASE_URL}/hmsweb/users/locations/" + location_id,
            {"geoEnabled": active},
        )
    ...
    def GetDevices(self, device_type=None, filter_provisioned=None):
        ...
        if device_type:
            devices = [
                device for device in devices if device.get("deviceType") in device_type
            ]

        if filter_provisioned is not None:
            if filter_provisioned:
                devices = [
                    device for device in devices if device.get("state") == "provisioned"
                ]
            else:
                devices = [
                    device for device in devices if device.get("state") != "provisioned"
    ...
    def GetDeviceCapabilities(self, device):
        ...
        return self.request.get(
            f"https://{self.BASE_URL}/resources/capabilities/"
            + model
            + "/"
            + model
            + "_"
            + device.get("interfaceVersion")
            + ".json",
            raw=True,
        )
    ...
    def GetLibraryMetaData(self, from_date, to_date):
        return self.request.post(
            f"https://{self.BASE_URL}/hmsweb/users/library/metadata",
            {"dateFrom": from_date, "dateTo": to_date},
        )

    def UpdateProfile(self, first_name, last_name):
        return self.request.put(
            f"https://{self.BASE_URL}/hmsweb/users/profile",
            {"firstName": first_name, "lastName": last_name},
        )

    def UpdatePassword(self, password):
        r = self.request.post(
            f"https://{self.BASE_URL}/hmsweb/users/changePassword",
    ...
    def RemoveFriend(self, email):
        ...
        """
        return self.request.post(
            f"https://{self.BASE_URL}/hmsweb/users/friends/remove", {"email": email}
        )
    ...
    def AddFriend(self, firstname, lastname, email, devices={}, admin=False):
        ...
        """
        return self.request.post(
            f"https://{self.BASE_URL}/hmsweb/users/friends",
            {
                "adminUser": admin,
                "firstName": firstname,
                "lastName": lastname,
    ...
    def ResendFriendInvite(self, friend):
        ...
        """
        return self.request.post(
            f"https://{self.BASE_URL}/hmsweb/users/friends", friend
        )

    def UpdateDeviceName(self, device, name):
        return self.request.put(
            f"https://{self.BASE_URL}/hmsweb/users/devices/renameDevice",
            {
                "deviceId": device.get("deviceId"),
                "deviceName": name,
    ...
    def UpdateDisplayOrder(self, body):
        ...
        """
        return self.request.post(
            f"https://{self.BASE_URL}/hmsweb/users/devices/displayOrder", body
        )
    ...
    def DeleteRecording(self, recording):
        ...
        """
        return self.request.post(
            f"https://{self.BASE_URL}/hmsweb/users/library/recycle",
            {
                "data": [
                    {
                        "createdDate": recording.get("createdDate"),
                        "utcCreatedDate": recording.get("createdDate"),
    ...
    def StartStream(self, basestation, camera):
        ...
        def callback(self, event):
            if (
                event.get("from") == basestation.get("deviceId")
                and event.get("resource") == "cameras/" + camera.get("deviceId")
                and event.get("properties", {}).get("activityState")
                == "userStreamActive"
            ):
    ...
    def StopStream(self, basestation, camera):
        ...
        def callback(self, event):
            if (
                ...
                return (
                    stream_url_dict["url"].replace("rtsp://", "rtsps://")
                    if stream_url_dict
                    else None
                )
            return None
    ...
    def TriggerStreamSnapshot(self, basestation, camera):
        ...
        def trigger(self):
            self.request.post(
                f"https://{self.BASE_URL}/hmsweb/users/devices/takeSnapshot",
                {
                    "xcloudId": camera.get("xCloudId"),
                    "parentId": camera.get("parentId"),
        ...
        def callback(self, event):
            if (
                event.get("deviceId") == camera.get("deviceId")
                and event.get("resource") == "mediaUploadNotification"
            ):
    ...
    def TriggerFullFrameSnapshot(self, basestation, camera):
        ...
        def trigger(self):
            self.request.post(
                "https://my.arlo.com/hmsweb/users/devices/fullFrameSnapshot",
                {
                    "to": camera.get("parentId"),
                    "from": self.user_id + "_web",
                    "resource": "cameras/" + camera.get("deviceId"),
        ...
        def callback(self, event):
            if (
                event.get("from") == basestation.get("deviceId")
                and event.get("resource") == "cameras/" + camera.get("deviceId")
                and event.get("action") == "fullFrameSnapshotAvailable"
            ):
    ...
    def StartRecording(self, basestation, camera):
        ...
        self.request.post(
            f"https://{self.BASE_URL}/hmsweb/users/devices/startRecord",
            {
                "xcloudId": camera.get("xCloudId"),
                "parentId": camera.get("parentId"),
                "deviceId": camera.get("deviceId"),
    ...
    def StopRecording(self, camera):
        ...
        """
        return self.request.post(
            f"https://{self.BASE_URL}/hmsweb/users/devices/stopRecord",
            {
                "xcloudId": camera.get("xCloudId"),
                "parentId": camera.get("parentId"),
                "deviceId": camera.get("deviceId"),
