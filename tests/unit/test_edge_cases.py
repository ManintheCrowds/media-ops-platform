...
class TestEmptyListHandling:
    ...
    @pytest.mark.asyncio
    async def test_list_cameras_empty(self, db_session):
    ...
    @pytest.mark.asyncio
    async def test_list_encoders_empty(self, db_session):
        """Test listing encoders when none exist."""
        service = VideoEncoderService()
        
        encoders = await service.list_encoders(db_session)
        
        assert isinstance(encoders, list)
        assert len(encoders) == 0
    
    @pytest.mark.asyncio
    async def test_discover_cameras_empty_result(self, db_session):
        ...
        with patch.object(service, '_get_arlo_client', return_value=mock_client):
            cameras = await service.discover_cameras(db_session)
            
            assert isinstance(cameras, list)
            assert len(cameras) == 0
    
    @pytest.mark.asyncio
    async def test_discover_encoders_empty_result(self, db_session):
...
class TestNoneValueHandling:
    ...
    @pytest.mark.asyncio
    async def test_register_camera_with_none_fields(self, db_session):
    ...
    @pytest.mark.asyncio
    async def test_register_encoder_with_none_fields(self, db_session):
    ...
    @pytest.mark.asyncio
    async def test_get_library_with_none_dates(self, db_session):
...
class TestBoundaryConditions:
    ...
    @pytest.mark.asyncio
    async def test_camera_id_max_int(self, db_session):
        ...
        with pytest.raises(ArloCameraError):
            await service.get_camera(db_session, max_id)
    
    @pytest.mark.asyncio
    async def test_encoder_id_max_int(self, db_session):
        ...
        with pytest.raises(EncoderError):
            await service.get_encoder(db_session, max_id)
    
    @pytest.mark.asyncio
    async def test_record_duration_zero(self, db_session):
    ...
    @pytest.mark.asyncio
    async def test_record_duration_very_large(self, db_session):
