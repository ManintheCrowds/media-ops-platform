    ...
    mock.GetDevices.return_value = [
        {
            ...
            "state": "provisioned",
            "properties": {"armed": True, "batteryLevel": 85, "signalStrength": 90},
        }
    ]
...
class TestArloServiceDiscover:
    ...
    @pytest.mark.asyncio
    async def test_discover_cameras_success(
        self, arlo_service, db_session, mock_arlo_client
    ):
        """Test successful camera discovery."""
        with patch.object(
            arlo_service, "_get_arlo_client", return_value=mock_arlo_client
        ):
    ...
    @pytest.mark.asyncio
    async def test_discover_cameras_filters_by_type(
        self, arlo_service, db_session, mock_arlo_client
    ):
...
class TestArloServiceListGet:
    ...
    async def test_list_cameras(self, arlo_service, db_session):
        ...
        camera1 = ArloCamera(
            base_station_id=station.id, name="Camera 1", device_id="CAM001"
        )
        camera2 = ArloCamera(
            base_station_id=station.id, name="Camera 2", device_id="CAM002"
        )
...
class TestArloServiceArmDisarm:
    ...
    @pytest.mark.asyncio
    async def test_disarm_camera_success(
        self, arlo_service, db_session, mock_arlo_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_arm_camera_client_error(
        self, arlo_service, db_session, mock_arlo_client
    ):
