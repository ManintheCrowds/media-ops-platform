...
def mock_aja_client():
    ...
    mock.get_full_status = AsyncMock(
        return_value={"recording": False, "streaming": False, "status": "online"}
    )
    return mock
...
def mock_config(tmp_path):
    ...
    config = EncoderConfig(
        storage_path=str(storage_path), encoder_network_scan_range="192.168.1.0/24"
    )
    return config
...
class TestVideoEncoderServiceGetClient:
    ...
    def test_get_client_success(self, encoder_service, db_session, mock_aja_client):
        ...
        encoder = VideoEncoder(name="Test Encoder", ip_address="192.168.1.100", port=80)
        db_session.add(encoder)
        db_session.commit()

        with patch(
            "services.video_encoder.encoder_service.AJAHELOClient",
            return_value=mock_aja_client,
        ):
    ...
    def test_get_client_caches_instance(
        self, encoder_service, db_session, mock_aja_client
    ):
...
class TestVideoEncoderServiceDiscover:
    ...
    @pytest.mark.asyncio
    async def test_discover_encoders_uses_default_range(
...
class TestVideoEncoderServiceRegister:
    ...
    async def test_register_encoder_new(self, encoder_service, db_session):
        ...
        # Verify in database
        encoder = (
            db_session.query(VideoEncoder).filter_by(ip_address="192.168.1.100").first()
        )
    ...
    async def test_register_encoder_existing(self, encoder_service, db_session):
        ...
        encoder = VideoEncoder(
            name="Old Name", ip_address="192.168.1.100", status=EncoderStatus.OFFLINE
        )
        db_session.add(encoder)
        db_session.commit()
...
class TestVideoEncoderServiceRecord:
    ...
    @pytest.mark.asyncio
    async def test_record_stream_success(
        self, encoder_service, db_session, mock_aja_client
    ):
        """Test successfully starting recording."""
        encoder = VideoEncoder(
            name="Test Encoder", ip_address="192.168.1.100", is_recording=False
        )
        db_session.add(encoder)
        db_session.commit()

        with patch.object(
            encoder_service, "_get_encoder_client", return_value=mock_aja_client
        ):
            result = await encoder_service.record_stream(
                db_session, encoder.id, "rtmp://source.example.com/stream", duration=60
            )
    ...
    async def test_record_stream_already_recording(self, encoder_service, db_session):
        ...
        encoder = VideoEncoder(
            name="Test Encoder", ip_address="192.168.1.100", is_recording=True
        )
        db_session.add(encoder)
        db_session.commit()
        ...
        with pytest.raises(EncoderRecordingError) as exc_info:
            await encoder_service.record_stream(
                db_session, encoder.id, "rtmp://source.example.com/stream"
            )
    ...
    async def test_record_stream_not_found(self, encoder_service, db_session):
        ...
        with pytest.raises(EncoderError):
            await encoder_service.record_stream(
                db_session, 999, "rtmp://source.example.com/stream"
            )

    @pytest.mark.asyncio
    async def test_record_stream_client_error(
        self, encoder_service, db_session, mock_aja_client
    ):
...
class TestVideoEncoderServiceStatus:
    ...
    @pytest.mark.asyncio
    async def test_get_encoder_status_success(
        self, encoder_service, db_session, mock_aja_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_encoder_status_recording(
        self, encoder_service, db_session, mock_aja_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_encoder_status_connection_error(
        self, encoder_service, db_session, mock_aja_client
    ):
