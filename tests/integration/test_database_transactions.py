...
import pytest
from unittest.mock import patch, AsyncMock, Mock, Mock
...
class TestTransactionRollback:
    ...
    async def test_register_encoder_rollback_on_error(self, db_session):
        ...
        encoder_data = {"ip_address": "192.168.1.100", "name": "Test Encoder"}
        ...
        encoders = (
            db_session.query(VideoEncoder).filter_by(ip_address="192.168.1.100").all()
        )
        ...
        with patch.object(service, "_get_encoder_client", return_value=mock_client):
            with pytest.raises(Exception):
                await service.record_stream(
                    db_session, encoder.id, source_url="rtmp://example.com/stream"
                )
...
class TestTransactionCommit:
    ...
    async def test_register_encoder_commits_successfully(self, db_session):
        ...
        encoder_data = {"ip_address": "192.168.1.101", "name": "Test Encoder 2"}

        result = await service.register_encoder(db_session, encoder_data)

        # Verify encoder exists in database
        encoder = (
            db_session.query(VideoEncoder).filter_by(ip_address="192.168.1.101").first()
        )
