---
name: OpenGrimoire intent brainstorm
overview: Run a structured `/brainstorm` session (WHAT, not HOW) that triages your canonical backlog in `portfolio-harness/.cursor/state` against two intent layers—OpenGrimoire alignment items and higher-level local hardware/stack goals—and defines an audit scope for OpenGrimoire context monitoring plus observability for the incoming local-proto hardware stack.
todos:
  ...
  - id: write-brainstorm-md
    content: Create docs/brainstorms/2026-03-22-opengrimoire-intent-triage-hardware-obs.md with template sections
    status: completed
...
# Brainstorm: Todo triage, OpenGrimoire context/intent audit, hardware-stack observation
...
- **Tactical “intent” surface:** OpenGrimoire `**alignment_context_items`** via `[GET/POST/PATCH /api/alignment-context](D:/portfolio-harness/OpenGrimoire/docs/agent/ALIGNMENT_CONTEXT_API.md)` (title, body, tags, priority, status, `linked_node_id`).
- **Context visualization:** Brain map / context graph from `[build_brain_map.py](D:/portfolio-harness/.cursor/scripts/build_brain_map.py)` → `public/brain-map-graph.json` and routes `/context-atlas`, `/brain-map`; inventory in `[OPENGRIMOIRE_SYSTEMS_INVENTORY.md](D:/portfolio-harness/OpenGrimoire/docs/OPENGRIMOIRE_SYSTEMS_INVENTORY.md)` (SCP-skipped files, multi-`CURSOR_STATE_DIRS`, vault roots).
...
flowchart LR
  ...
  subgraph oa [OpenGrimoire monitoring surfaces]
...
3. **Phase 2 — Approaches (2–3):** Compare lightweight options, for example:
  ...
  - **C — Split observability:** Separate “human alignment” (OpenGrimoire UI/API) from “machine health” (Ollama/Jetson/NAS metrics — out of OpenGrimoire unless you explicitly bridge them).
   Lead with a **YAGNI** recommendation (likely A or B first, C scoped as a second track).
4. **Phase 3 — Capture:** Write `[docs/brainstorms/2026-03-22-opengrimoire-intent-triage-hardware-obs.md](D:/portfolio-harness/docs/brainstorms/2026-03-22-opengrimoire-intent-triage-hardware-obs.md)` (or similar slug) using your brainstorming template: **What we’re building**, **Why this approach**, **Key decisions**, **Open questions**, **Resolved questions** (move items as you go).
...
| **OpenGrimoire “audit”**    | What “working” means: graph freshness (regen cadence), SCP skips, API auth (`x-alignment-context-key`), E2E coverage (`npm run verify:e2e`), gaps between graph nodes and alignment links. |
...
- **Any future implementation** stays separate: OpenGrimoire app code under `[OpenGrimoire/](D:/portfolio-harness/OpenGrimoire)`, harness scripts under `[.cursor/scripts/](D:/portfolio-harness/.cursor/scripts)`, local-proto under `[local-proto/docs/](D:/portfolio-harness/local-proto/docs)` — do not conflate in a single “monitoring” blob without a design note.
