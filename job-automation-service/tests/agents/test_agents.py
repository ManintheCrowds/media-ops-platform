    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        for method, path, data in endpoints:
            try:
                ...
                results["failure_count"] += 1
                results["errors"].append({path: "Request timeout"})
            except httpx.ConnectError:
                results["failure_count"] += 1
                results["errors"].append({path: "Connection error - service may not be running"})
            except Exception as e:
                results["failure_count"] += 1
    ...
    for job in test_jobs:
        try:
            ...
            if letter and len(letter) > 100:  # Basic quality check
                ...
                results["failure_count"] += 1
                results["errors"].append({
                    job["title"]: f"Letter too short or empty: {len(letter) if letter else 0} chars"
                })
                
        except Exception as e:
            logger.error(f"Cover letter generation failed: {e}")
            results["failure_count"] += 1
            results["errors"].append({job["title"]: str(e)})
...
async def scheduler_test_agent(task: AgentTask) -> Dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        ...
        try:
            ...
            response = await client.post(f"{base_url}/api/v1/scheduler/start")
            results["tests_run"] += 1
            ...
            if response.status_code in [200, 201]:
                ...
                results["failure_count"] += 1
                results["errors"].append({
                    "scheduler_start": f"Status {response.status_code}"
                })
        except Exception as e:
            results["failure_count"] += 1
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                results["failure_count"] += 1
                results["errors"].append({
                    "scheduler_stop": f"Status {response.status_code}"
                })
        except Exception as e:
            results["failure_count"] += 1
